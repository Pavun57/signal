...
export interface AffiliationWrite {
   ...
   * under the company the verdict was about: they are filed somewhere else
   * entirely, or filed nowhere with the mismatch already on record.
   *
   ...
   * unchanged. This one says "this person is not a contact here at all". A
   * test fake that does not model the field reads `undefined` and takes the
   * conservative branch.
   */
  notAtJudgedOrg?: boolean;
  /**
   * True when an ATTACHING write was refused because the person is filed under
   * a different company whose evidence outranks this claim.
   *
   * The attach-side twin of notAtJudgedOrg. Without it a caller cannot tell
   * "already here, with stronger evidence" (a contact, keep them) from "works
   * somewhere else, with stronger evidence" (not a contact here), and
   * campaign-linked strangers as contacts of this company.
   */
...
export async function recordAffiliation(
  ...
  // Two attempts, because the UPDATE below is scoped to the snapshot this read
  // returns: a concurrent writer makes it match nothing, and the honest
  // response to that is to re-read and re-run the guard against what actually
  // won, not to blindly re-apply a decision made about a row that no longer
  // exists. A second miss means the row is being actively rewritten, and
  // refusing is safer than fighting.
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // A failed query is not a missing person. Reporting it as person_not_found
    // had callers filing the refusal under "unchanged" and storing evidence
    // strings about a write that was never attempted.
    if (readError) {
      console.error(
        `[affiliation] person lookup failed for ${personId}: ${readError.message}`,
      );
      return {
        written: false,
        reason: `person lookup failed: ${readError.message}`,
      };
    }
    if (!person) return { written: false, reason: "person_not_found" };

    const existingSource =
      person.affiliation_source as AffiliationSource | null;
    ...
    const humanOverride =
      source === "user_entered" || source === "user_rejected";
    ...
    if (detaching) {
      ...
      if (
        ...
        person.organization_id !== detachedFrom
      ) {
        return {
          written: false,
      ...
      // A person attached NOWHERE has nothing to detach, but the verdict is
      // still worth remembering: freshly discovered rejected candidates are
      // created with no organization, and refusing to record the mismatch left
      // affiliation_detached_from unset, so the re-attach guard (case 2a)
      // never armed and the next run could re-attach the same wrong person on
      // the same evidence. Record it when it is strictly stronger than what is
      // on file; refuse it as not-a-contact-here otherwise, so a repeat
      // judgement is not mistaken for "keep them in the list, unchanged".
      if (
        person.organization_id === null &&
        !humanOverride &&
        incoming <= existingWeight
      ) {
        return {
          written: false,
    ...
    if (!humanOverride) {
      if (detaching) {
        ...
        // scope check above proves that, or they are filed nowhere and the
        // nowhere branch above already required strictly stronger evidence).
        // Requires STRICTLY stronger evidence than what put them there, so a
        // scraped snapshot at 0.8 cannot overrule the company's own team page
        // at 0.9.
        ...
        if (incoming === existingWeight && existingSource) {
          return {
            written: false,
            reason: "already_recorded_at_same_strength",
          };
        }
        ...
        //
        // Flagged, because to the caller this refusal means something no other
        // one does: the person works somewhere else, on stronger evidence, so
        // they are not a contact at the org being attached to.
        return {
          written: false,
          reason: "not_stronger_than_existing",
    ...
    //
    // The extra predicates are a compare-and-swap: the guard above evaluated a
    // snapshot, so the write only applies while the row still IS that snapshot.
    // `.select("id")` is how PostgREST reports whether anything matched.
    let write = supabase
      .from("people")
      .update({
      ...
      .eq("id", personId);
    write = existingSource
      ? write.eq("affiliation_source", existingSource)
      : write.is("affiliation_source", null);
    write = person.organization_id
      ? write.eq("organization_id", person.organization_id)
      : write.is("organization_id", null);
    ...
    // Matched nothing: a concurrent writer moved the row between our read and
    // this write. Loop back and judge the incoming claim against what won.
    if (!updated || updated.length === 0) continue;

    return { written: true };
  }

  return { written: false, reason: "concurrent_update" };
}
