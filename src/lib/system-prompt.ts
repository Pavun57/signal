...
- **\`findContacts\` returns \`verifiedCount\`, \`uncertainCount\` and \`rejectedAsWrongCompany\`; \`searchPeople\` returns \`uncertainCount\` and \`rejectedAsWrongCompany\`.** Rejected means the evidence positively placed that person at a *different* company — commonly a similarly-named one — so they were stored unattached. Uncertain means the evidence didn't settle it, usually because their LinkedIn headline never names an employer, which is normal at small companies.
