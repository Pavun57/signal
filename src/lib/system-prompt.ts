...
- **\`findContacts\` and \`searchPeople\` return \`verifiedCount\`, \`uncertainCount\` and \`rejectedAsWrongCompany\`.** Rejected means the evidence positively placed that person at a *different* company — commonly a similarly-named one — so they were stored unattached. Uncertain means the evidence didn't settle it, usually because their LinkedIn headline never names an employer, which is normal at small companies.
...
- **Emails must be verified before they can be sent.** \`findEmail\` returns a \`verification\` field: \`deliverable\` is confirmed, \`risky\` usually means a catch-all domain that accepts every address and proves nothing, \`unchecked\` means no email provider is configured on this instance. Only \`deliverable\` (or an address the user typed, or one a previous send confirmed) can be emailed.
