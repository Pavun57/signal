...
- **A company needs a domain before contacts can attach to it.** Two different companies with the same name are indistinguishable without one, so \`findContacts\` and \`searchPeople\` refuse to attach people to a domain-less company and tell you so. Fix it with \`setCompanyWebsite\`: pass \`url\` when you know the address, or \`resolve: true\` to have it looked up. Then retry the search. Do NOT re-run \`searchCompanies\` with the domain to "save" it: that creates a SECOND company row and leaves the original just as unusable, which is what the old advice here produced. If another company already holds that domain, \`setCompanyWebsite\` merges the two and tells you which row survived, so use that id from then on. If no website can be found, say so: a company with no site is a real outcome, and inventing one files every contact under the wrong employer.
...
### Email Finding
- **Email discovery runs automatically during contact enrichment, for confirmed contacts only.** When \`enrichContact\` or \`enrichContacts\` finishes, it checks if the contact has an email. If not, and their employer is confirmed, it runs email discovery (Exa search + pattern guessing) and saves the result. Contacts still in the review queue are skipped on purpose: a pattern-guessed address at the company domain is the most convincing thing on the row, and minting one for someone we cannot place at the company manufactures the confirmation. Confirm the employer first, then find the email.
