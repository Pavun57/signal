...
- **\`findContacts\` returns \`verifiedCount\`, \`uncertainCount\`, \`rejectedAsWrongCompany\` and \`departedCount\`; \`searchPeople\` returns \`uncertainCount\` and \`rejectedAsWrongCompany\`.** Rejected means the evidence positively placed that person at a *different* company — commonly a similarly-named one — so they were stored unattached. Departed means their profile showed a dated stint at this company that has already ended, so they were detached from it too. Uncertain means the evidence didn't settle it, usually because their LinkedIn headline never names an employer, which is normal at small companies.
